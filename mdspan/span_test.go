...
func TestSpan(test *testing.T) {
	cases := []spanCase{
		...
		blocks("image/direct_link_with_separating_newline.md", spans{
			...
			md.Image{AltText: "link", URL: "/img.png", RawEnd: md.Raw{{0, bb("]\n(/img.png)")}}},
			md.End{},
		}),
		...
		lines("image/incomplete.md", spans{
			...
			md.End{},
			md.Image{ReferenceID: "ref undefined", AltText: "ref undefined", RawEnd: md.Raw{{2, bb("]")}}},
			md.End{},
			md.Image{ReferenceID: "ref 1", AltText: "ref 1", RawEnd: md.Raw{{3, bb("]")}}},
			md.End{},
		}, head(8)),
		blocks("image/link_text_with_newline.md", spans{
			// FIXME(akavel): here, we should fix splitting file to Regions
